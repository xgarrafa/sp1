...
/// The `PcsProof` type parameter represents the PCS proof type (defaults to `()` when no PCS is used).
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(bound(
    serialize = "PcsProof: Serialize",
...
pub struct ZkCnstrProof<GC: ZkIopCtx, PcsProof = ()> {
...
impl<GC: ZkIopCtx, PD> ZkProverContext<GC, PD> {
    ...
    pub fn prove<RNG, P>(mut self, rng: &mut RNG, pcs_prover: Option<&P>) -> ZkProof<GC, P::Proof>
    where
        RNG: rand::CryptoRng + rand::Rng,
        rand::distributions::Standard: rand::distributions::Distribution<GC::EF>,
